...
router.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    const [openDeals, wonThisMonth, lostThisMonth, stageGroups, ownerGroups, weightedGroups, wonWeeks] = await Promise.all([
      prisma.deal.count({ where: openWhere }),
      prisma.deal.count({ where: { deletedAt: null, stage: DealStage.WON, closedAt: { gte: monthStart, lt: nextMonthStart }, ...access } }),
      prisma.deal.count({ where: { deletedAt: null, stage: DealStage.LOST, closedAt: { gte: monthStart, lt: nextMonthStart }, ...access } }),
      prisma.deal.groupBy({ by: ['stage'], where: openWhere, _count: { _all: true } }),
      prisma.deal.groupBy({ by: ['ownerId'], where: openWhere, _count: { _all: true } }),
      Promise.all(openStages.map((stage) => prisma.deal.aggregate({ where: { ...openWhere, stage }, _sum: { value: true } }))),
      prisma.deal.findMany({
        where: { deletedAt: null, stage: DealStage.WON, closedAt: { gte: eightWeekStart, lt: weekEnd }, ...access },
    ...
    const weightedPipeline = weightedGroups.reduce((total, group, index) => {
      ...
      return total.add(stageTotal.mul(String(stageProbabilities[openStages[index]])))
    }, new Prisma.Decimal(0))
    const stageCounts = new Map(stageGroups.map((group) => [group.stage, group._count._all]))
    const ownerIds = ownerGroups.map((group) => group.ownerId)
    const owners = ownerIds.length === 0 ? [] : await prisma.user.findMany({ where: { id: { in: ownerIds } }, select: { id: true, email: true } })
    ...
    return res.json({
      ...
      data: {
        ...
        openDealsByStage: openStages.map((stage) => ({ stage, count: stageCounts.get(stage) || 0 })),
        openDealsByOwner: ownerGroups.map((group) => ({ ownerId: group.ownerId, ownerEmail: ownerEmails.get(group.ownerId) || 'Unknown owner', count: group._count._all })),
        wonPerWeek,
      },
    })
  } catch (error) {
