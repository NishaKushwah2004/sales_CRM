    ...
    <main className="min-h-screen bg-slate-950 px-6 py-12 text-slate-100">
      <div className="mx-auto max-w-6xl">
        <nav className="flex flex-wrap items-center gap-3 text-sm">
          ...
          <Link className="text-slate-300 hover:text-sky-300" to="/companies">
            Companies
          </Link>
          <Link className="text-slate-300 hover:text-sky-300" to="/deals">
            Deals
          </Link>
          <Link
          ...
          >
            Past Due
            {alertCount > 0 && (
              <span className="ml-1 rounded-full bg-amber-400 px-1.5 py-0.5 text-xs font-semibold text-slate-950">
                {alertCount}
              </span>
            )}
          </Link>
        ...
        <p className="mt-8 text-sm font-semibold uppercase tracking-[0.2em] text-sky-400">
          Sales overview
        </p>
        <h1 className="mt-3 text-3xl font-bold">Dashboard</h1>
        <p className="mt-2 text-slate-400">
          A server-calculated view of your accessible pipeline.
        </p>
        {error && (
          <p className="mt-6 rounded bg-red-950 p-3 text-red-200">{error}</p>
        )}
        {loading ? (
          ...
          <>
            ...
            <section
              id="past-due"
              className="mt-6 rounded border border-amber-800 bg-amber-950/40 p-5"
            ...
            >
              ...
              {alerts.length === 0 ? (
                ...
                <ul className="mt-4 space-y-3">
                  {alerts.map((alert) => (
                    <li
                      className="rounded border border-amber-800/70 bg-slate-950/40 p-4"
                      key={alert.dealId}
                    >
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-amber-200/80">
                            {alert.companyName} ·{" "}
                            {stageLabels[alert.stage] || alert.stage}
                          </p>
                          <p className="mt-1 text-sm text-amber-200/70">
                            Expected close: {alert.expectedCloseDate}
                          </p>
                        </div>
                        {alert.canDismiss && (
                          <button
                            className="rounded border border-amber-500 px-3 py-1 text-sm text-amber-100"
            ...
            <section className="mt-6 grid gap-6 lg:grid-cols-2">
              ...
              <article className="rounded border border-slate-800 bg-slate-900 p-5">
                <h2 className="text-lg font-semibold">Open deals by owner</h2>
                {ownerData.length === 0 ? (
                  <p className="mt-5 text-slate-400">
                    No open deals are available.
                  </p>
                ) : (
                  <div className="mt-5 h-64">
                    <ResponsiveContainer width="100%" height="100%">
            ...
            <article className="mt-6 rounded border border-slate-800 bg-slate-900 p-5">
              <h2 className="text-lg font-semibold">Won deals per week</h2>
