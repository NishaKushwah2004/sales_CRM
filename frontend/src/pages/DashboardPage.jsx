  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setAlertCount(alertResponse.data.data.count ?? alertResponse.data.data.alerts.length)
      } catch (requestError) {
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-6 py-12 text-slate-100">
      <div className="mx-auto max-w-6xl">
        <nav className="flex flex-wrap items-center gap-3 text-sm"><Link className="text-sky-300" to="/">Dashboard</Link><Link className="text-slate-300 hover:text-sky-300" to="/companies">Companies</Link><Link className="text-slate-300 hover:text-sky-300" to="/deals">Deals</Link><Link className="text-slate-300 hover:text-sky-300" to="/dashboard#past-due">Past Due{alertCount > 0 && <span className="ml-1 rounded-full bg-amber-400 px-1.5 py-0.5 text-xs font-semibold text-slate-950">{alertCount}</span>}</Link><button className="text-slate-400 hover:text-slate-100" type="button" onClick={handleLogout}>Log out</button></nav>
        ...
        {loading ? <p className="mt-8 text-slate-300">Loading dashboard...</p> : <>
          ...
          <section id="past-due" className="mt-6 rounded border border-amber-800 bg-amber-950/40 p-5" aria-labelledby="alerts-heading">
